...
class LauncherClientImpl(private val launcher: me.lal.lawnchair.Launcher, targetPackage: String, overlayEnabled: Boolean) : ILauncherClient {
    ...
    constructor(launcher: me.lal.lawnchair.Launcher, overlayEnabled: Boolean) : this(launcher, "com.google.android.googlequicksearchbox", overlayEnabled)

    private fun applyWindowToken() {
        if (!isConnected || !me.lal.lawnchair.Utilities.getPrefs(launcher).showGoogleNowTab) {
            return
        }
    ...
    override fun onAttachedToWindow() {
        if (!destroyed && me.lal.lawnchair.Utilities.getPrefs(launcher).showGoogleNowTab) {
            setWindowAttrs(launcher.window.attributes)
        }
    }

    override fun onDestroy() {
        if (me.lal.lawnchair.Utilities.getPrefs(launcher).showGoogleNowTab) {
    ...
    fun reconnect() {
        if (destroyed || state != 0 || !me.lal.lawnchair.Utilities.getPrefs(launcher).showGoogleNowTab) {
            return
        }
