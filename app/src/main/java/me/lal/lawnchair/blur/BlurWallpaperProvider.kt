...
class BlurWallpaperProvider(context: Context) {
    ...
    private fun updateBlurRadius() {
        blurRadius = me.lal.lawnchair.Utilities.getPrefs(context).blurRadius.toInt() / DOWNSAMPLE_FACTOR
        blurRadius = Math.max(1, Math.min(blurRadius, 25))
    }

    private val enabledFlag: Int
        get() = me.lal.lawnchair.Utilities.getPrefs(context).blurMode

    private fun updateWallpaper() {
        val launcher = me.lal.lawnchair.LauncherAppState.getInstance().launcher
        val enabled = mWallpaperManager.wallpaperInfo == null && me.lal.lawnchair.Utilities.getPrefs(context).enableBlur
        ...
        if (me.lal.lawnchair.Utilities.getPrefs(context).enableVibrancy) {
    ...
    val tintColor: Int
        get() = me.lal.lawnchair.Utilities.resolveAttributeData(context, R.attr.blurTintColor)

    fun updateAsync() {
        me.lal.lawnchair.Utilities.THREAD_POOL_EXECUTOR.execute(mUpdateRunnable)
    }
    ...
    companion object {
        ...
        fun applyBlurBackground(activity: Activity) {
            ...
            var color = me.lal.lawnchair.Utilities.resolveAttributeData(activity, R.attr.blurTintColor)
        ...
        val instance: BlurWallpaperProvider
            get() = me.lal.lawnchair.LauncherAppState.getInstance().launcher.blurWallpaperProvider
    }
}
