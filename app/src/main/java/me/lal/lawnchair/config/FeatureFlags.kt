...
object FeatureFlags {
    ...
    const val KEY_LOAD_PIN = "pref_load_pin"
    ...
    fun pullDownAction(context: Context): Int {
        me.lal.lawnchair.Utilities.getPrefs(context).migratePullDownPref(context)
    ...
    fun applyDarkTheme(activity: Activity) {
        me.lal.lawnchair.Utilities.getPrefs(activity).migrateThemePref(activity)
