...
import me.lal.lawnchair.util.IconNormalizer
import java.util.*
...
class ClockIconDrawable(val context: Context, val adaptive: Boolean) : Drawable() {
    ...
    private fun getBackground(width: Int, height: Int): Bitmap {
        ...
        return me.lal.lawnchair.Utilities.addShadowToIcon(bitmap, width)
    }
    ...
    companion object {
        fun create(context: Context): Drawable {
            if (me.lal.lawnchair.Utilities.ATLEAST_OREO) {
            ...
            } else if (me.lal.lawnchair.Utilities.ATLEAST_NOUGAT) {
                return Wrapper(me.lal.lawnchair.pixelify.AdaptiveIconDrawableCompat(
                        ColorDrawable(context.resources.getColor(R.color.blue_grey_100)),
    ...
    class Wrapper(val drawable: Drawable, val adaptive: Boolean) : me.lal.lawnchair.FastBitmapDrawable(), Callback {
        val canvas = Canvas()
        ...
        override fun onBoundsChange(bounds: Rect) {
            ...
            if (adaptive) {
                ...
                val scale = me.lal.lawnchair.util.IconNormalizer.getInstance().getScale(drawable, null)
                ...
                if (me.lal.lawnchair.Utilities.ATLEAST_OREO) {
                    ...
                    me.lal.lawnchair.pixelify.AdaptiveIconDrawableCompat(ColorDrawable(Color.WHITE), ColorDrawable(Color.WHITE), false).apply {
                        setBounds(inset, inset, width - inset, height - inset)
                    }.draw(canvas)
                }
                shadow = BitmapDrawable(me.lal.lawnchair.Utilities.getShadowForIcon(bitmap, width))
