...
import me.lal.lawnchair.DropTarget;
import me.lal.lawnchair.Launcher;
import me.lal.lawnchair.LauncherAnimUtils;
import me.lal.lawnchair.LauncherModel;
import me.lal.lawnchair.dragndrop.DragLayer;
import me.lal.lawnchair.dragndrop.DragOptions;
import me.lal.lawnchair.AbstractFloatingView;
import me.lal.lawnchair.BubbleTextView;
import me.lal.lawnchair.DragSource;
import me.lal.lawnchair.DropTarget;
import me.lal.lawnchair.FastBitmapDrawable;
import me.lal.lawnchair.ItemInfo;
import me.lal.lawnchair.Launcher;
import me.lal.lawnchair.LauncherAnimUtils;
import me.lal.lawnchair.LauncherModel;
import me.lal.lawnchair.LauncherSettings;
import me.lal.lawnchair.LogAccelerateInterpolator;
...
import me.lal.lawnchair.accessibility.ShortcutMenuAccessibilityDelegate;
import me.lal.lawnchair.allapps.AllAppsIconRowView;
import me.lal.lawnchair.anim.PropertyListBuilder;
import me.lal.lawnchair.anim.PropertyResetListener;
import me.lal.lawnchair.badge.BadgeInfo;
import me.lal.lawnchair.config.FeatureFlags;
import me.lal.lawnchair.dragndrop.DragController;
import me.lal.lawnchair.dragndrop.DragLayer;
import me.lal.lawnchair.dragndrop.DragOptions;
import me.lal.lawnchair.graphics.IconPalette;
import me.lal.lawnchair.graphics.TriangleShape;
import me.lal.lawnchair.notification.NotificationItemView;
import me.lal.lawnchair.notification.NotificationKeyData;
import me.lal.lawnchair.shortcuts.DeepShortcutManager;
import me.lal.lawnchair.shortcuts.DeepShortcutView;
...
public class PopupContainerWithArrow extends AbstractFloatingView implements DragSource,
        ...
        for (int i = 0; i < numItems; i++) {
            ...
            if (itemTypeToPopulate.isShortcut) {
                if (mShortcutsItemView == null) {
                    ...
                    applyCorrectBackground(mShortcutsItemView);
                ...
                applyCorrectBackground(item);
                if (item instanceof NotificationItemView && spacing == 0) {
                    item.setElevation(0);
                    item.setBackgroundColor(Color.WHITE);
                    ((NotificationItemView) item).showSectionDivider(mIsAboveIcon);
                } else if (shouldAddBottomMargin && spacing != 0) {
    ...
    private void addItem(LayoutInflater inflater, View item) {
        if (mTheme.getWrapInMain()) {
            if (mMainItemView == null) {
                ...
                applyCorrectBackground(mMainItemView, true);
    ...
    private void applyCorrectBackground(View item) {
        applyCorrectBackground(item, false);
    }
    ...
    public Animator reduceNotificationViewHeight(int heightToRemove, int duration) {
        ...
        if (mMainItemView != null) {
            mNotificationItemView.setElevation(0);
            mReduceHeightAnimatorSet.play(mNotificationItemView.animateHeightRemoval(heightToRemove));
            mReduceHeightAnimatorSet.play(mMainItemView.animateHeightRemoval(heightToRemove, mIsAboveIcon));
            for (int i = 0; i < mMainItemView.getItemContainer().getChildCount(); i++) {
                final PopupItemView itemView = (PopupItemView) mMainItemView.getItemContainer().getChildAt(i);
